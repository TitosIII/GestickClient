...
const SearchComponent = ({ baseDeDatos, renderCart, getExistences }) => {
  ...
  const results = !search ? baseDeDatos : baseDeDatos.filter((dato) => dato.PrNombre.toLowerCase().includes(search.toLocaleLowerCase()));
  ...
  return (
    <>
      <div className="buscar2">
        <input className="Buscador" type="text" placeholder="Buscar" onChange={searcher} />
      </div>
      {results.map(({ PrURLimg, PrNombre, PrPrecio, PrExistencias, idProductos }) => (
